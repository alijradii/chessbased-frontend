...
import { currentFenAtom, gameHistoryAtom, historyIndexAtom, currentTurnAtom, gameStatusAtom } from "@/lib/chess-store"
    ...
    <Card className="w-full">
      ...
      <CardContent className="space-y-4">
        <div>
          ...
          <p className="text-sm text-muted-foreground">{currentTurn === "w" ? "White" : "Black"}</p>
        </div>

        <div>
          <h3 className="text-sm font-semibold mb-1">Game Status</h3>
          <p className="text-sm text-muted-foreground capitalize">{gameStatus}</p>
        </div>
        ...
        <div>
          ...
          <p className="text-xs text-muted-foreground font-mono break-all">{currentFen}</p>
        </div>
        ...
        {gameHistory.length > 0 && (
          <div>
            ...
            <div className="max-h-40 overflow-y-auto space-y-1">
              {gameHistory.slice(0, historyIndex).map((move, index) => (
                <div key={index} className="text-xs text-muted-foreground flex items-center gap-2">
                  <span className="font-semibold w-8">{Math.floor(index / 2) + 1}.</span>
